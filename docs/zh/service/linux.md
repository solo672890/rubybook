...
`以下示例使用fail2ban封锁恶意扫描网站ip`

1. 确保已安装 Fail2Ban 及 pyinotify（高性能日志监控后端）
````
# Debian/Ubuntu
sudo apt install fail2ban python3-pyinotify -y

# CentOS/RHEL
sudo yum install fail2ban python3-inotify -y

# 验证 pyinotify 可用
/usr/bin/python3 -c "import pyinotify; print('OK')"
````

2. 创建自定义过滤器
````
vim /etc/fail2ban/filter.d/nginx-403-404.conf

[Definition]
# 匹配 Nginx 默认 combined 日志格式中的 403 和 404 状态码
failregex = ^<HOST> -.*"(GET|POST|HEAD).*" (403|404) .*$
ignoreregex =
````

⚠️ 重要提示：failregex 必须与您实际的 Nginx 日志格式完全匹配。如果您的 Nginx 使用了非标准日志格式，请先用 fail2ban-regex 测试：

`sudo fail2ban-regex /var/log/nginx/access.log /etc/fail2ban/filter.d/nginx-403-404.conf`


3. 配置 Jail

````
vim /etc/fail2ban/jail.d/nginx-403-404.conf
...
logpath   = /var/log/nginx/access.log
 /var/log/nginx/access1.log 
backend   = pyinotify

# === 封禁策略 ===
# 允许的最大失败次数
maxretry  = 8
# 统计时间窗口（秒），60秒内触发10次即封禁
findtime  = 60
# 封禁时长（秒），1day
bantime   = 86400
# 或使用 nftables[type=multiport]
banaction = iptables-multiport

# === 渐进式封禁===
# 启用后，重复违规者封禁时间会指数级增长
bantime.increment = true
# 每次翻倍：1bantime → 2bantime → 4bantime → bantime...
bantime.factor    = 2
# 最大封禁7天
...
# 确认 jail 状态及 backend
sudo fail2ban-client status nginx-403-404

# 确认 pyinotify 已加载（查看最新日志）
grep -i "Initiated 'pyinotify'" /var/log/fail2ban.log | tail -1
````

5. 日常运维命令速查
````
查看封禁列表	    sudo fail2ban-client get nginx-403-404 banned
手动封禁IP	    sudo fail2ban-client set nginx-403-404 banip 1.2.3.4
手动解封IP	    sudo fail2ban-client set nginx-403-404 unbanip 1.2.3.4
实时查看封禁日志	sudo tail -f /var/log/fail2ban.log | grep Ban
````
