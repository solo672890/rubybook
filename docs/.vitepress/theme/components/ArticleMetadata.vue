...
const releaseDate = computed(
    () => page.value.frontmatter.releaseTime
)
...
<template>
  <div class="word">
    <p class="ht">
      <Badge v-if="page.frontmatter.original" class="mrl yc" type="danger" text="原创" />
      <Badge v-else class="mrl yc" type="warning" text="转载/参考" />
      <span  v-if="releaseDate">
            ...
            <span class="mrl"> 发布于: {{ releaseDate }} </span>
      </span>
      ...
      <span class="mrl">字数: {{ wordCount }} 字</span>
      ...
      <span > 时长: {{ readTime }} 分钟</span>
        <linkcard
            v-if="!page.frontmatter.original && page.frontmatter.fromTitle"
            :url="page.frontmatter.fromUrl??''" :title="page.frontmatter.fromTitle??''"  :description="page.frontmatter.fromUrl??''" logo="/svg/stackblitz_logo.svg"/>
